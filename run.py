...
def debug_track_seq(args, model, img_paths, gts):
    ...
    # eval_img_path, eval_gt = img_paths[4], gts[4]
    t = time.time()
    ...
    model.fit(train_data=train_iter, eval_data=None,
              ...
              eval_metric=mx.metric.CompositeEvalMetric(
                  [extend.PR(0.5), extend.RR(0.5), extend.TrackTopKACC(10, 0.5)]),
    ...
    # test.check_internal(model, train_iter)
    a = model.score(datahelper.get_val_iter(
        datahelper.get_val_data(img_paths[3], pre_region=gts[3], gt=gts[3])),
        mx.metric.CompositeEvalMetric([extend.PR(0.5), extend.RR(0.5), extend.TrackTopKACC(10, 0.5)]))
...
def debug_track_on_OTB():
    ...
    # a = datahelper.get_train_iter(datahelper.get_train_data(img_paths[0], gts[0]))
...
def parse_args():
    ...
    parser.add_argument('--lr_step', default=32 * 3, help='every 121 num for one epoch', type=int)
    parser.add_argument('--lr_factor', default=0.4, help='20 times will be around 0.1', type=float)
    parser.add_argument('--momentum', default=0.9, type=float)
    parser.add_argument('--lr_stop', default=1e-4, type=float)
    parser.add_argument('--lr_offline', default=1.1e-2, help='base learning rate', type=float)
